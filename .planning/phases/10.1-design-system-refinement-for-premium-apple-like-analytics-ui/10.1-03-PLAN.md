...
must_haves:
  truths:
    - "MASTER.md contains the actual implemented token values (not the original stale values)"
    - "MASTER.md is the canonical reference that Phase 11–14 planning agents will use"
    - "All conflicts with original MASTER.md (light background, Fira fonts, blue primary) are overridden"
    - "Visual review confirms every component matches its CONTEXT.md spec"
  artifacts:
    - path: "design-system/clippers-command-center/MASTER.md"
      provides: "Canonical design system: tokens, typography scale, spacing, component specs, chart palette"
  key_links:
    - from: "design-system/clippers-command-center/MASTER.md"
      to: "app/globals.css"
      via: "MASTER.md token values must match implemented globals.css values exactly"
      pattern: "090B10|141620|chart-1"
---

<objective>
Rewrite MASTER.md to be the canonical design system reference reflecting everything implemented in Plans 01 and 02. Then perform a human visual review of all components against the CONTEXT.md specification.

Purpose: MASTER.md is the input that Phase 11–14 planning agents read when designing new components. If it contains stale values (the original had light backgrounds and Fira fonts), future phases will plan against wrong specs. This plan makes MASTER.md the authoritative source of truth.
Output: MASTER.md rewritten with the complete implemented design system. Human visual sign-off confirming the premium aesthetic goal is met.
</objective>
...
@.planning/phases/10.1-design-system-refinement-for-premium-apple-like-analytics-ui/10.1-02-SUMMARY.md

<interfaces>
<!-- Read the current MASTER.md before overwriting — required by tool protocol -->
<!-- Location: design-system/clippers-command-center/MASTER.md -->

<!-- The new MASTER.md must document (at minimum): -->
<!-- 1. Design philosophy and references -->
<!-- 2. Color tokens (all --custom-property values) -->
<!-- 3. Typography scale (5 levels with size/weight/tracking) -->
<!-- 4. Spacing system (--space-1 through --space-12) -->
<!-- 5. Component specifications (TopNav, StatCard, BoxScoreTable, charts, skeletons, StaleBanner) -->
<!-- 6. Chart palette (--chart-1 through --chart-4) -->
<!-- 7. Anti-patterns to avoid in future phases -->
</interfaces>
</context>
...
<task type="auto">
  <name>Task 1: Rewrite MASTER.md with canonical implemented design system</name>
  <files>design-system/clippers-command-center/MASTER.md</files>
  <action>
Read the current `design-system/clippers-command-center/MASTER.md` first (required). Then overwrite it completely with a new canonical document reflecting the Phase 10.1 implemented design system.

The new MASTER.md must be written as a reference for future Claude planning agents (Phases 11–14). It should be precise, scannable, and authoritative. Override all conflicting original values — do not preserve light-mode tokens, Fira fonts, or generic blue primary.
...
**Do NOT** preserve any conflicting original values. The full file is a replacement, not an amendment.
  </action>
  <verify>
    <automated>cd /Users/luke/CCC && grep -c "090B10\|chart-1\|0.6875rem\|surface-alt\|Anti-Pattern" design-system/clippers-command-center/MASTER.md</automated>
  </verify>
  <done>MASTER.md is fully rewritten with: design philosophy section, complete color token table (background #090B10, surface #141620, surface-alt #1C2030, all chart tokens, positive/negative), 5-level typography scale, spacing table, all 6 component specifications, anti-patterns section. No conflicting original values remain (no light backgrounds, no Fira fonts, no blue primary).</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
Complete Phase 10.1 visual refinement across all components:
- Plan 01: globals.css tokens, TopNav (48px, glass), NavLinks (1px underline), StatCard (plain div, dominant value), StatCardSkeleton
- Plan 02: BoxScoreTable (hairline borders, 36px rows, no outer card), LineChartWrapper and BarChartWrapper (horizontal gridlines, 0.08 opacity, chart token colors, surface-alt tooltip), BoxScoreSkeleton, ChartSkeleton, StaleBanner (amber-950/40)
- Plan 03: MASTER.md fully rewritten as canonical reference
  </what-built>
  <how-to-verify>
Start the dev server: `cd /Users/luke/CCC && npm run dev`

Visit http://localhost:3000 and check each of the following:

**1. Overall atmosphere (10 seconds)**
- Background should be near-black with a blue undertone — significantly darker than before
- Page should feel calm and premium, not like a generic admin dashboard

**2. Navigation (TopNav + NavLinks)**
- Nav bar is noticeably shorter — slim, Raycast-like, not chunky
- Bottom border is barely visible — hairline, not a heavy divider
- Nav glass blur is visible when scrolling (if page has enough content)
- Wordmark "CCC" is tight, bold, tracking-widest
- Active nav link has a thin 1px red underline — NOT a thick bar
- Inactive links are dimmed (medium gray) with foreground on hover

**3. StatCard (visit any page that renders stat cards)**
- The number/value is the biggest, boldest element — immediately draws the eye
- Label above the value is tiny, ALL CAPS, clearly subordinate
- No drop shadow or hover lift when moving mouse over cards
- Card background is subtly lighter than the page background (surface elevation visible)
- Border is barely visible — hairline only

**4. BoxScoreTable (visit any page with a box score)**
- Table has NO outer card border — it floats in its section
- Header row text is very small, ALL CAPS, muted — clearly labels, not data
- Data rows are compact (36px) — dense but readable
- Row separator lines are barely there — horizontal only, no vertical lines
- Hovering a row gives an extremely subtle highlight (barely perceptible)

**5. Charts (if a chart is visible)**
- Chart grid lines are nearly invisible — solid hairlines, not dashed, horizontal only
- Chart colors are muted (blue-400, emerald-400 palette) — not saturated primaries
- Tooltips have a dark surface background (darker than chart background)

**6. Pixel check (optional, high confidence)**
- Browser DevTools → Computed → nav header → height should be exactly 48px
- Browser DevTools → Inspect :root → --background should be #090B10

Report: "approved" if the overall aesthetic reads as premium/restrained analytics UI (Apple Sports / Linear / Raycast tone). Describe any component that still looks heavy, over-decorated, or off-spec.
  </how-to-verify>
...
<verification>
After Task 1:
- `grep -c "090B10\|chart-1\|Anti-Pattern\|0.6875rem" /Users/luke/CCC/design-system/clippers-command-center/MASTER.md` returns >= 4

After human visual review (Task 2):
- User has confirmed the aesthetic reads as premium dark analytics — calm, restrained, information-dense
- All components meet their CONTEXT.md spec
</verification>

<success_criteria>
- MASTER.md fully rewritten with implemented values — no stale/conflicting content
- MASTER.md covers: philosophy, color tokens, typography scale, spacing, 6 component specs, anti-patterns, Tailwind v4 patterns
- Human visual review passed: user confirms aesthetic meets Apple Sports / Linear / Raycast reference quality
- Phase 10.1 complete — Phase 11 (Live Game Dashboard) can proceed using MASTER.md as its design reference
</success_criteria>

<output>
After completion, create `.planning/phases/10.1-design-system-refinement-for-premium-apple-like-analytics-ui/10.1-03-SUMMARY.md` following the summary template.
</output>
